        ...
        <form id="edit-task-form-{{ $task->id }}" method="post" action="{{ route('rooms.tasks.update', [$room, $task]) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-6">
                ...
                <div x-data="taskAutocomplete({ suggestUrl: @js($suggestUrl) })" x-init="q = @js($task->name)"
                     @task-chosen="
                        const sel = document.getElementById('type-{{ $task->id }}'); 
                        if (sel && $event.detail.task.type) sel.value = $event.detail.task.type;
                        const inst = document.getElementById('instructions-{{ $task->id }}');
                        ...
                        const sporadic = document.getElementById('is_sporadic-{{ $task->id }}');
