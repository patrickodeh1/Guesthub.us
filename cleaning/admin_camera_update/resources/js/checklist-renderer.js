...
export default function checklistRenderer(config = {}) {
    return {
        ...
        renderPhotosRoomByRoom() {
            ...
            const skippedRooms = this.sessionData.session?.skipped_rooms || []; const minPhotos = skippedRooms.includes(room.id) ? 0 : Number(room.min_photos ?? 2); const photos = photosByRoom[room.id] || []; let incompleteTasks = []; rooms.forEach(r => { if (skippedRooms.includes(r.id)) return;
        ...
        renderPhotos() {
            ...
            // Check for incomplete mandatory tasks across all rooms let incompleteTasks = []; const skippedRooms = this.sessionData.session?.skipped_rooms || []; rooms.forEach(room => { if (skippedRooms.includes(room.id)) return;
                const roomTasksArray = Array.isArray(room.tasks) ? room.tasks : Object.values(room.tasks || {});
                const roomTasks = roomTasksArray.filter(t => room.room_tasks.includes(t.id));
            ...
            let pendingPhotos = []; rooms.forEach(room => { if (skippedRooms.includes(room.id)) return; const count = photoCounts[room.id] || 0;
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex flex-col gap-1">
                                    ...
                                    <template x-if="taskType === 'inventory' && !${checked}">
                                        ...
                                        <div class="mt-2 text-[10px] w-full" x-show="${hasInstructions} || ${hasMedia} || itemPhotos.length > 0 || noteValue.length > 0" x-cloak>
