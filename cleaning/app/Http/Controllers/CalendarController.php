...
class CalendarController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($acting !== 'housekeeper') {
            ...
            $icalProps = Property::query()->active()
            ...
            foreach ($icalProps as $prop) {
                ...
                foreach ($urls as $item) {
                    ...
                    // Delete checkouts that were cancelled/removed from this specific calendar feed
                    if (count($fetchedUids) > 0) {
                        PropertyCheckout::where('property_id', $prop->id)
                            ->where('source', $item['source'])
                            ->whereNotIn('uid', $fetchedUids)
                            ->delete();
                    } else {
                        // If feed is completely empty, delete all for this source
                        PropertyCheckout::where('property_id', $prop->id)
                            ->where('source', $item['source'])
                            ->delete();
                    }
            ...
            foreach ($checkouts as $checkout) {
                ...
                if (!$exists) {
                    $unscheduledByDate->push([
                        ...
                        'property_name' => $checkout->property?->name ?? 'Unknown Property',
