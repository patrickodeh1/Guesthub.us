<?php

namespace App\Http\Controllers;

use App\Http\Requests\ChecklistNoteRequest;
use App\Http\Requests\ChecklistToggleRequest;
use App\Models\ChecklistItem;
use App\Models\ChecklistItemPhoto;
use App\Models\CleaningSession;
use App\Models\CleanerInstructionFamiliarity;
use App\Models\Room;
use App\Models\Setting;
use App\Models\Task;
use App\Services\ImageTimestampService;
use App\Services\SmsNotificationService;
use App\Services\PersistentPhotoStorage;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Storage;

class ChecklistController extends Controller
{
    public function toggle(ChecklistToggleRequest $request, CleaningSession $session, Room $room, Task $task)
    {
        // Authorization: Allow if user is housekeeper assigned to this session or admin/owner
        $user = auth()->user();
        $isAssignedHousekeeper = $session->housekeeper_id === $user->id;
        $isOwnerOrAdmin = $user->hasRole(['admin', 'owner', 'company']);
        
        if (!$isAssignedHousekeeper && !$isOwnerOrAdmin) {
            return response()->json(['message' => 'Unauthorized access to this session.'], 403);
        }
    
        Log::info("ChecklistController@toggle: Session {$session->id}, Room {$room->id}, Task {$task->id}");
        // RELAXED CHECK: If item exists, allow toggle
        $itemExists = ChecklistItem::where('session_id', $session->id)
            ->where('room_id', $room->id)
            ->where('task_id', $task->id)
            ->exists();

        if (! $itemExists) {
            // Explicitly check room-property relationship for NEW items
            if (! $room->properties()->where('properties.id', $session->property_id)->exists()) {
                Log::error("ChecklistController@toggle: Room {$room->id} not attached to property {$session->property_id}");
                return response()->json(['message' => 'Room is not attached to this property.'], 404);
            }

             // Explicitly check task-room relationship for NEW items
            if (! $room->tasks()->where('tasks.id', $task->id)->exists()) {
                Log::error("ChecklistController@toggle: Task {$task->id} not attached to room {$room->id}");
                return response()->json(['message' => 'Task is not attached to this room.'], 404);
            }
        }

        // Create if missing (unique on session_id+room_id+task_id is recommended at DB level)
        $item = ChecklistItem::firstOrCreate(
            [
                'session_id' => $session->id,
                'room_id'    => $room->id,
                'task_id'    => $task->id,
            ],
            [
                'user_id' => auth()->id(),
                'checked' => false,
            ]
        );

        if ($request->has('force_checked')) {
            $nowChecked = $request->boolean('force_checked');
        } else {
            $nowChecked = ! $item->checked;
        }

        if ($nowChecked) {
            $trainingService = app(\App\Services\TrainingValidationService::class);
            $pendingTraining = $trainingService->getPendingDuringTaskTraining($session, $task, $user, $item);

            if (!empty($pendingTraining)) {
                if ($request->expectsJson()) {
                    return response()->json([
                        'success' => false,
                        'code' => 'TRAINING_REQUIRED',
                        'message' => 'Complete the required training before marking this task as complete.',
                        'task_id' => $task->id,
                        'pending_training' => $pendingTraining,
                    ], 403);
                }
                return back()->with('error', 'You must complete the required training before marking this task as complete.');
            }
        }

        $item->update([
            'checked'    => $nowChecked,
            'checked_at' => $nowChecked ? now() : null,
            'user_id'    => auth()->id(),
            'quantity'   => $request->input('quantity'),
        ]);

        if ($nowChecked && !$item->checked) {
            activity('checklist')
                ->performedOn($session)
                ->causedBy(auth()->user())
                ->withProperties([
                    'property_id' => $session->property_id,
                    'room_id' => $room->id,
                    'task_id' => $task->id,
                    'task_type' => $task->type,
                ])
                ->log("Completed checklist task: {$task->name}");
        }
        if ($request->expectsJson()) {
            return response()->json([
                'success' => true,
                'checked' => $nowChecked,
                'item'    => [
                    'id'         => $item->id,
                    'checked'    => $item->checked,
                    'checked_at' => $item->checked_at?->toIso8601String(),
                ],
            ]);
        }

        return back();
    }

    /**
     * Toggle property-level task (no room).
     */
    public function togglePropertyTask(ChecklistToggleRequest $request, CleaningSession $session, Task $task)
    {
        // Authorization: Allow if user is housekeeper assigned to this session or admin/owner
        $user = auth()->user();
        $isAssignedHousekeeper = $session->housekeeper_id === $user->id;
        $isOwnerOrAdmin = $user->hasRole(['admin', 'owner', 'company']);
        
        if (!$isAssignedHousekeeper && !$isOwnerOrAdmin) {
            return response()->json(['message' => 'Unauthorized access to this session.'], 403);
        }
        
        Log::info("ChecklistController@togglePropertyTask: Session {$session->id}, Task {$task->id}");
        // RELAXED CHECK: If the checklist item ALREADY exists for this session, we allow toggling it,
        // even if the task was theoretically removed from the property definition after session start.
        $itemExists = ChecklistItem::where('session_id', $session->id)
            ->where('task_id', $task->id)
            ->whereNull('room_id')
            ->exists();

        if (! $itemExists) {
            // RELAXED CHECK: We allow toggling if the session and task exist. 
            // We only log a warning if it's not strictly attached to the property definitions,
            // to avoid blocking work due to sync delays.
            $isAttached = $session->property->propertyTasks()->where('tasks.id', $task->id)->exists();
            if (!$isAttached) {
                Log::warning("ChecklistController@togglePropertyTask: Task {$task->id} not found as property-level task for property {$session->property_id}, but allowing toggle.");
            }
        }

        // Create if missing
        $item = ChecklistItem::firstOrCreate(
            [
                'session_id' => $session->id,
                'room_id'    => null, // Property-level tasks have no room
                'task_id'    => $task->id,
            ],
            [
                'user_id' => auth()->id(),
                'checked' => false,
            ]
        );

        if ($request->has('force_checked')) {
            $nowChecked = $request->boolean('force_checked');
        } else {
            $nowChecked = ! $item->checked;
        }

        if ($nowChecked) {
            $trainingService = app(\App\Services\TrainingValidationService::class);
            $pendingTraining = $trainingService->getPendingDuringTaskTraining($session, $task, $user, $item);

            if (!empty($pendingTraining)) {
                if ($request->expectsJson()) {
                    return response()->json([
                        'success' => false,
                        'code' => 'TRAINING_REQUIRED',
                        'message' => 'Complete the required training before marking this task as complete.',
                        'task_id' => $task->id,
                        'pending_training' => $pendingTraining,
                    ], 403);
                }
                return back()->with('error', 'You must complete the required training before marking this task as complete.');
            }
        }

        $item->update([
            'checked'    => $nowChecked,
            'checked_at' => $nowChecked ? now() : null,
            'user_id'    => auth()->id(),
            'quantity'   => $request->input('quantity'),
        ]);

        if ($nowChecked && !$item->checked) {
            activity('checklist')
                ->performedOn($session)
                ->causedBy(auth()->user())
                ->withProperties([
                    'property_id' => $session->property_id,
                    'room_id' => null,
                    'task_id' => $task->id,
                    'task_type' => $task->type,
                ])
                ->log("Completed property task: {$task->name}");
        }

        // Return JSON for AJAX requests
        if ($request->expectsJson()) {
            return response()->json([
                'success' => true,
                'checked' => $nowChecked,
                'item'    => [
                    'id'         => $item->id,
                    'checked'    => $item->checked,
                    'checked_at' => $item->checked_at?->toIso8601String(),
                ],
            ]);
        }

        return back();
    }

    /**
     * Add/update a note for a specific (session, room, task).
     */
    public function note(ChecklistNoteRequest $request, CleaningSession $session, Room $room, Task $task)
    {
        $this->assertRoomOnSessionProperty($room, $session);
        
        // Relaxed check for task-room
        $itemExists = ChecklistItem::where('session_id', $session->id)
            ->where('room_id', $room->id)
            ->where('task_id', $task->id)
            ->exists();

        if (!$itemExists) {
            $this->assertTaskAttachedToRoom($task, $room);
        }

        $item = ChecklistItem::firstOrCreate(
            [
                'session_id' => $session->id,
                'room_id'    => $room->id,
                'task_id'    => $task->id,
            ],
            [
                'user_id' => auth()->id(),
                'checked' => false,
            ]
        );

        $item->update([
            'note'    => $request->validated('note'),
            'user_id' => auth()->id(),
        ]);

        // Trigger SMS notification for task note
        if ($request->validated('note')) {
            try {
                SmsNotificationService::sendTaskNote($session, $task, $request->validated('note'));
            } catch (\Throwable $e) {
                Log::warning('SMS notification failed for task note: ' . $e->getMessage());
            }
        }

        // Return JSON for AJAX requests
        if ($request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Note saved successfully',
                'item'    => [
                    'id'   => $item->id,
                    'note' => $item->note,
                ],
            ]);
        }

        return back();
    }

    /**
     * Add/update a note for property-level task (no room).
     */
    public function notePropertyTask(ChecklistNoteRequest $request, CleaningSession $session, Task $task)
    {
        // RELAXED CHECK: If the checklist item ALREADY exists for this session, we allow adding notes,
        // even if the task was theoretically removed from the property definition.
        $itemExists = ChecklistItem::where('session_id', $session->id)
            ->where('task_id', $task->id)
            ->whereNull('room_id')
            ->exists();

        if (! $itemExists) {
            abort_unless(
                $session->property->propertyTasks()->where('tasks.id', $task->id)->exists(),
                404,
                'Task not found as property-level task for this session.'
            );
        }

        $item = ChecklistItem::firstOrCreate(
            [
                'session_id' => $session->id,
                'room_id'    => null, // Property-level tasks have no room
                'task_id'    => $task->id,
            ],
            [
                'user_id' => auth()->id(),
                'checked' => false,
            ]
        );

        $item->update([
            'note'    => $request->validated('note'),
            'user_id' => auth()->id(),
        ]);

        // Trigger SMS notification for task note
        if ($request->validated('note')) {
            try {
                SmsNotificationService::sendTaskNote($session, $task, $request->validated('note'));
            } catch (\Throwable $e) {
                Log::warning('SMS notification failed for task note: ' . $e->getMessage());
            }
        }

        // Return JSON for AJAX requests
        if ($request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Note saved successfully',
                'item'    => [
                    'id'   => $item->id,
                    'note' => $item->note,
                ],
            ]);
        }

        return back();
    }

    // ---------------------------
    // Guards for new relationships
    // ---------------------------

    /** Ensure the given room is attached to the session's property (property_room pivot). */
    private function assertRoomOnSessionProperty(Room $room, CleaningSession $session): void
    {
        $propertyId = $session->property_id;

        abort_unless(
            $room->properties()->where('properties.id', $propertyId)->exists(),
            404
        );
    }

    /** Ensure the given task is attached to the given room (room_task pivot). */
    private function assertTaskAttachedToRoom(Task $task, Room $room): void
    {
        abort_unless(
            $room->tasks()->where('tasks.id', $task->id)->exists(),
            404
        );
    }

    /**
     * Upload a photo for a specific task (room-level).
     */
    public function taskPhoto(Request $request, CleaningSession $session, Room $room, Task $task)
    {
        $this->assertRoomOnSessionProperty($room, $session);
        // RELAXED CHECK
        $itemExists = ChecklistItem::where('session_id', $session->id)
            ->where('room_id', $room->id)
            ->where('task_id', $task->id)
            ->exists();

        if (!$itemExists) {
            $this->assertTaskAttachedToRoom($task, $room);
        }

        $request->validate([
            'photo' => 'nullable|image|max:10240', // 10MB max
            'note' => 'nullable|string|max:1000',
        ]);

        // Get or create the checklist item
        $item = ChecklistItem::firstOrCreate(
            [
                'session_id' => $session->id,
                'room_id'    => $room->id,
                'task_id'    => $task->id,
            ],
            [
                'user_id' => auth()->id(),
                'checked' => false,
            ]
        );

        // If a photo was uploaded, process and store it
        if ($request->hasFile('photo')) {
            $photo = $this->processAndStorePhoto($request->file('photo'), $session);

            $photoRecord = ChecklistItemPhoto::create([
                'checklist_item_id' => $item->id,
                'room_id' => $room->id,
                'property_id' => $session->property_id,
                'path' => $photo['path'],
                'note' => $request->input('note'),
                'captured_at' => now(),
            ]);

            // Trigger SMS notification for first photo (once per session)
            try {
                SmsNotificationService::sendPhotoStarted($session);
            } catch (\Throwable $e) {
                Log::warning('SMS notification failed for photo started: ' . $e->getMessage());
            }

            if ($request->expectsJson()) {
                return response()->json([
                    'success' => true,
                    'message' => 'Photo uploaded successfully',
                    'photo' => [
                        'id'          => $photoRecord->id,
                        'url'         => $photoRecord->url,
                        'note'        => $photoRecord->note,
                        'captured_at' => $photoRecord->captured_at?->toIso8601String(),
                    ],
                ]);
            }
            return back()->with('success', 'Photo uploaded successfully');
        }

        // Note-only submission (no photo)
        if ($request->filled('note')) {
            $item->update([
                'note'    => $request->input('note'),
                'user_id' => auth()->id(),
            ]);

            // Trigger SMS notification for task note
            try {
                SmsNotificationService::sendTaskNote($session, $task, $request->input('note'));
            } catch (\Throwable $e) {
                Log::warning('SMS notification failed for task note: ' . $e->getMessage());
            }
        }

        if ($request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Note saved successfully',
                'item'    => [
                    'id'   => $item->id,
                    'note' => $item->note,
                ],
            ]);
        }

        return back()->with('success', 'Note saved successfully');
    }

    /**
     * Upload a photo for a property-level task.
     */
    public function propertyTaskPhoto(Request $request, CleaningSession $session, Task $task)
    {
        // RELAXED CHECK
        $itemExists = ChecklistItem::where('session_id', $session->id)
            ->where('task_id', $task->id)
            ->whereNull('room_id')
            ->exists();

        if (!$itemExists) {
            abort_unless(
                $session->property->propertyTasks()->where('tasks.id', $task->id)->exists(),
                404,
                'Task not found as property-level task for this session.'
            );
        }

        $request->validate([
            'photo' => 'nullable|image|max:10240', // 10MB max
            'note' => 'nullable|string|max:1000',
        ]);

        // Get or create the checklist item
        $item = ChecklistItem::firstOrCreate(
            [
                'session_id' => $session->id,
                'room_id'    => null,
                'task_id'    => $task->id,
            ],
            [
                'user_id' => auth()->id(),
                'checked' => false,
            ]
        );

        // If a photo was uploaded, process and store it
        if ($request->hasFile('photo')) {
            $photo = $this->processAndStorePhoto($request->file('photo'), $session);

            $photoRecord = ChecklistItemPhoto::create([
                'checklist_item_id' => $item->id,
                'room_id' => null,
                'property_id' => $session->property_id,
                'path' => $photo['path'],
                'note' => $request->input('note'),
                'captured_at' => now(),
            ]);

            // Trigger SMS notification for first photo (once per session)
            try {
                SmsNotificationService::sendPhotoStarted($session);
            } catch (\Throwable $e) {
                Log::warning('SMS notification failed for photo started: ' . $e->getMessage());
            }

            if ($request->expectsJson()) {
                return response()->json([
                    'success' => true,
                    'message' => 'Photo uploaded successfully',
                    'photo' => [
                        'id'          => $photoRecord->id,
                        'url'         => $photoRecord->url,
                        'note'        => $photoRecord->note,
                        'captured_at' => $photoRecord->captured_at?->toIso8601String(),
                    ],
                ]);
            }
            return back()->with('success', 'Photo uploaded successfully');
        }

        // Note-only submission (no photo)
        if ($request->filled('note')) {
            $item->update([
                'note'    => $request->input('note'),
                'user_id' => auth()->id(),
            ]);

            // Trigger SMS notification for task note
            try {
                SmsNotificationService::sendTaskNote($session, $task, $request->input('note'));
            } catch (\Throwable $e) {
                Log::warning('SMS notification failed for task note: ' . $e->getMessage());
            }
        }

        if ($request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Note saved successfully',
                'item'    => [
                    'id'   => $item->id,
                    'note' => $item->note,
                ],
            ]);
        }

        return back()->with('success', 'Note saved successfully');
    }

    /**
     * Delete a photo for a specific task (room-level).
     */
    public function deletePhoto(Request $request, CleaningSession $session, Room $room, Task $task, ChecklistItemPhoto $photo)
    {
        // First check that this photo belongs to an item that corresponds to the session/room/task
        $item = ChecklistItem::where('session_id', $session->id)
            ->where('room_id', $room->id)
            ->where('task_id', $task->id)
            ->first();

        if (!$item || $photo->checklist_item_id !== $item->id) {
            return response()->json(['success' => false, 'message' => 'Photo not found'], 404);
        }

        // Delete the file
        if (Storage::disk('public')->exists($photo->path)) {
            Storage::disk('public')->delete($photo->path);
        }
        if (class_exists(PersistentPhotoStorage::class)) {
            PersistentPhotoStorage::delete($photo->path);
        }

        $photo->delete();

        // Auto-untick if no photos remain
        if (ChecklistItemPhoto::where('checklist_item_id', $item->id)->count() === 0) {
            $item->update(['checked' => false, 'checked_at' => null]);
        }

        if ($request->expectsJson()) {
            return response()->json(['success' => true, 'message' => 'Photo deleted']);
        }

        return back()->with('success', 'Photo deleted successfully');
    }

    /**
     * Delete a photo for a property-level task.
     */
    public function deletePropertyTaskPhoto(Request $request, CleaningSession $session, Task $task, ChecklistItemPhoto $photo)
    {
        $item = ChecklistItem::where('session_id', $session->id)
            ->whereNull('room_id')
            ->where('task_id', $task->id)
            ->first();

        if (!$item || $photo->checklist_item_id !== $item->id) {
            return response()->json(['success' => false, 'message' => 'Photo not found'], 404);
        }

        // Delete the file
        if (Storage::disk('public')->exists($photo->path)) {
            Storage::disk('public')->delete($photo->path);
        }
        if (class_exists(PersistentPhotoStorage::class)) {
            PersistentPhotoStorage::delete($photo->path);
        }

        $photo->delete();

        // Auto-untick if no photos remain
        if (ChecklistItemPhoto::where('checklist_item_id', $item->id)->count() === 0) {
            $item->update(['checked' => false, 'checked_at' => null]);
        }

        if ($request->expectsJson()) {
            return response()->json(['success' => true, 'message' => 'Photo deleted']);
        }

        return back()->with('success', 'Photo deleted successfully');
    }

    /**
     * Process photo and add timestamp overlay.
     */
    private function processAndStorePhoto($file, CleaningSession $session): array
    {
        $session->loadMissing(['property', 'housekeeper']);
        
        $capturedAt = now();
        $propertyTz = $session->property->timezone ?? config('app.timezone');

        // Generate unique filename
        $filename = 'task_photo_' . $session->id . '_' . time() . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
        $path = 'checklist-photos/' . $session->id . '/' . $filename;

        // Ensure target directory exists (important for serverless temp roots)
        Storage::disk('public')->makeDirectory('checklist-photos/' . $session->id);

        // Store the file first
        $storedPath = Storage::disk('public')->putFileAs(
            'checklist-photos/' . $session->id,
            $file,
            $filename
        );

        if (!$storedPath) {
            throw new \RuntimeException('Unable to store uploaded checklist photo.');
        }

        // Apply timestamp overlay with full metadata
        $absolutePath = Storage::disk('public')->path($storedPath);
        ImageTimestampService::overlay(
            absolutePath: $absolutePath,
            when: $capturedAt,
            timezone: $propertyTz,
            propertyName: $session->property?->name,
            roomName: null, // Room name is resolved at the caller level if needed
            cleanerName: $session->housekeeper?->name,
            latitude: $session->start_latitude ? (float) $session->start_latitude : null,
            longitude: $session->start_longitude ? (float) $session->start_longitude : null,
        );
        // Persist to long-term storage when available (e.g., serverless deployments)
        if (class_exists(\App\Services\PersistentPhotoStorage::class)) {
            try {
                PersistentPhotoStorage::persistFromPublicDisk($storedPath);
            } catch (\Throwable $e) {
                Log::warning('ChecklistController: Unable to persist checklist photo', [
                    'stored_path' => $storedPath,
                    'error' => $e->getMessage(),
                ]);
            }
        }

        return [
            'path' => $storedPath,
            'timestamp' => $capturedAt->format('Y-m-d H:i:s'),
        ];
    }

    /**
     * Skip a room in the cleaning session.
     */
    public function skipRoom(Request $request, CleaningSession $session, Room $room)
    {
        $this->assertRoomOnSessionProperty($room, $session);

        $skipped = $session->skipped_rooms ?? [];
        if (!in_array($room->id, $skipped)) {
            $skipped[] = $room->id;
            $session->skipped_rooms = $skipped;
            $session->save();
        }

        if ($request->expectsJson()) {
            return response()->json(['success' => true, 'skipped_rooms' => $skipped]);
        }

        return back();
    }

    /**
     * Unskip a room in the cleaning session.
     */
    public function unskipRoom(Request $request, CleaningSession $session, Room $room)
    {
        $this->assertRoomOnSessionProperty($room, $session);

        $skipped = $session->skipped_rooms ?? [];
        if (in_array($room->id, $skipped)) {
            $skipped = array_values(array_diff($skipped, [$room->id]));
            $session->skipped_rooms = $skipped;
            $session->save();
        }

        if ($request->expectsJson()) {
            return response()->json(['success' => true, 'skipped_rooms' => $skipped]);
        }

        return back();
    }

    /**
     * Mark an instruction task as viewed (room-level).
     */
    public function viewInstruction(Request $request, CleaningSession $session, Room $room, Task $task)
    {
        $this->assertRoomOnSessionProperty($room, $session);
        
        $itemExists = ChecklistItem::where('session_id', $session->id)
            ->where('room_id', $room->id)
            ->where('task_id', $task->id)
            ->exists();

        if (!$itemExists) {
            $this->assertTaskAttachedToRoom($task, $room);
        }

        $item = ChecklistItem::firstOrCreate(
            [
                'session_id' => $session->id,
                'room_id'    => $room->id,
                'task_id'    => $task->id,
            ],
            [
                'user_id' => auth()->id(),
                'checked' => false,
            ]
        );

        $videoId = $request->input('video_id');

        if (!$videoId && !$item->instruction_viewed) {
            $item->update([
                'instruction_viewed' => true,
                'instruction_viewed_at' => now(),
            ]);
        }

        $user = auth()->user();
        
        // Legacy familiarity tracking (preserved for backward compatibility)
        $familiarity = CleanerInstructionFamiliarity::firstOrCreate(
            ['user_id' => $user->id, 'task_id' => $task->id, 'instructional_video_id' => $videoId],
            ['views_completed' => 0]
        );
        $familiarity->increment('views_completed');
        $familiarity->update(['last_viewed_at' => now()]);

        // NEW: Also create/update a TrainingCompletion record scoped to this session.
        // This is the authoritative source for During-Task training enforcement.
        if (!$videoId) {
            $trainingCompletion = \App\Models\TrainingCompletion::firstOrCreate(
                [
                    'user_id' => $user->id,
                    'task_id' => $task->id,
                    'instructional_video_id' => null,
                    'cleaning_session_id' => $session->id,
                    'training_version' => $task->training_version ?? 1,
                ],
                [
                    'property_id' => $session->property_id,
                    'status' => 'not_started',
                    'progress' => 0,
                    'views_completed' => 0,
                ]
            );
            $trainingCompletion->increment('views_completed');
            if ($trainingCompletion->status !== 'completed') {
                $trainingCompletion->update([
                    'status' => 'completed',
                    'progress' => 100,
                    'completed_at' => now(),
                ]);
            }
        }
        
        if ($videoId) {
            $video = \App\Models\InstructionalVideo::find($videoId);
            $requiredViews = $video && $video->is_required_during_task ? max(1, (int)$video->required_views) : 1;
        } else {
            if ($task->is_required_during_task) {
                $requiredViews = max(1, (int)$task->required_views);
            } else {
                $globalViews = (int) Setting::get('global_required_instruction_views', 3);
                $userViews = $user->preferences['required_instruction_views'] ?? null;
                $requiredViews = max(1, $userViews !== null ? (int) $userViews : $globalViews);
            }
        }

        activity('checklist')
            ->performedOn($session)
            ->causedBy($user)
            ->withProperties([
                'property_id' => $session->property_id,
                'room_id' => $room->id,
                'task_id' => $task->id,
                'video_id' => $videoId,
            ])
            ->log("Viewed instruction for task: {$task->name}");
            
        activity('checklist')
            ->performedOn($session)
            ->causedBy($user)
            ->withProperties([
                'property_id' => $session->property_id,
                'task_id' => $task->id,
                'video_id' => $videoId,
                'views_completed' => $familiarity->views_completed,
            ])
            ->log("Familiarity Increased");

        if ($familiarity->views_completed == $requiredViews) {
            activity('checklist')
                ->performedOn($session)
                ->causedBy($user)
                ->withProperties([
                    'property_id' => $session->property_id,
                    'task_id' => $task->id,
                    'video_id' => $videoId,
                ])
                ->log("Cleaner Became Familiar");
        }
        
        $viewsCompleted = $familiarity->views_completed;
        $isFamiliar = $requiredViews <= 0 || $viewsCompleted >= $requiredViews;

        return response()->json([
            'success' => true,
            'views_completed' => $viewsCompleted,
            'required_views' => $requiredViews,
            'is_familiar' => $isFamiliar
        ]);
    }

    /**
     * Mark an instruction task as viewed (property-level).
     */
    public function viewPropertyTaskInstruction(Request $request, CleaningSession $session, Task $task)
    {
        $itemExists = ChecklistItem::where('session_id', $session->id)
            ->where('task_id', $task->id)
            ->whereNull('room_id')
            ->exists();

        if (!$itemExists) {
            abort_unless(
                $session->property->propertyTasks()->where('tasks.id', $task->id)->exists(),
                404,
                'Task not found as property-level task for this session.'
            );
        }

        $item = ChecklistItem::firstOrCreate(
            [
                'session_id' => $session->id,
                'room_id'    => null,
                'task_id'    => $task->id,
            ],
            [
                'user_id' => auth()->id(),
                'checked' => false,
            ]
        );

        $videoId = $request->input('video_id');

        if (!$videoId && !$item->instruction_viewed) {
            $item->update([
                'instruction_viewed' => true,
                'instruction_viewed_at' => now(),
            ]);
        }

        $user = auth()->user();
        
        // Legacy familiarity tracking (preserved for backward compatibility)
        $familiarity = CleanerInstructionFamiliarity::firstOrCreate(
            ['user_id' => $user->id, 'task_id' => $task->id, 'instructional_video_id' => $videoId],
            ['views_completed' => 0]
        );
        $familiarity->increment('views_completed');
        $familiarity->update(['last_viewed_at' => now()]);

        // NEW: Also create/update a TrainingCompletion record scoped to this session.
        // This is the authoritative source for During-Task training enforcement.
        if (!$videoId) {
            $trainingCompletion = \App\Models\TrainingCompletion::firstOrCreate(
                [
                    'user_id' => $user->id,
                    'task_id' => $task->id,
                    'instructional_video_id' => null,
                    'cleaning_session_id' => $session->id,
                    'training_version' => $task->training_version ?? 1,
                ],
                [
                    'property_id' => $session->property_id,
                    'status' => 'not_started',
                    'progress' => 0,
                    'views_completed' => 0,
                ]
            );
            $trainingCompletion->increment('views_completed');
            if ($trainingCompletion->status !== 'completed') {
                $trainingCompletion->update([
                    'status' => 'completed',
                    'progress' => 100,
                    'completed_at' => now(),
                ]);
            }
        }
        
        if ($videoId) {
            $video = \App\Models\InstructionalVideo::find($videoId);
            $requiredViews = $video && $video->is_required_during_task ? max(1, (int)$video->required_views) : 1;
        } else {
            if ($task->is_required_during_task) {
                $requiredViews = max(1, (int)$task->required_views);
            } else {
                $globalViews = (int) Setting::get('global_required_instruction_views', 3);
                $userViews = $user->preferences['required_instruction_views'] ?? null;
                $requiredViews = max(1, $userViews !== null ? (int) $userViews : $globalViews);
            }
        }

        activity('checklist')
            ->performedOn($session)
            ->causedBy($user)
            ->withProperties([
                'property_id' => $session->property_id,
                'room_id' => null,
                'task_id' => $task->id,
                'video_id' => $videoId,
            ])
            ->log("Viewed instruction for property task: {$task->name}");
            
        activity('checklist')
            ->performedOn($session)
            ->causedBy($user)
            ->withProperties([
                'property_id' => $session->property_id,
                'task_id' => $task->id,
                'video_id' => $videoId,
                'views_completed' => $familiarity->views_completed,
            ])
            ->log("Familiarity Increased");

        if ($familiarity->views_completed == $requiredViews) {
            activity('checklist')
                ->performedOn($session)
                ->causedBy($user)
                ->withProperties([
                    'property_id' => $session->property_id,
                    'room_id' => null,
                    'task_id' => $task->id,
                    'video_id' => $videoId,
                ])
                ->log("Cleaner Became Familiar");
        }

        $viewsCompleted = $familiarity->views_completed;
        $isFamiliar = $requiredViews <= 0 || $viewsCompleted >= $requiredViews;

        return response()->json([
            'success' => true,
            'views_completed' => $viewsCompleted,
            'required_views' => $requiredViews,
            'is_familiar' => $isFamiliar
        ]);
    }
}