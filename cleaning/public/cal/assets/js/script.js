...
// Global settings for blocked dates appearance
let globalSettings = {
  blockedColor: '#ef4444',
  showOverlaps: true
};
...
/* ════════════════════════════════════════
   GENERATE AVAILABLE EVENTS (BARS)
════════════════════════════════════════ */
...
/* ════════════════════════════════════════
   GENERATE BLOCKED EVENTS (BARS) - NEW
════════════════════════════════════════ */
function generateBlockedEvents() {
  const blockedEvents = [];
  const enabledSources = sources.filter(s => s.enabled === true && s.showBlockedDates === true);

  console.log(`🚫 Generating blocked events for ${enabledSources.length} sources...`);

  enabledSources.forEach(src => {
    if (!src.blockedDates || src.blockedDates.length === 0) return;

    src.blockedDates.forEach(block => {
      // Create one event per blocked range
      blockedEvents.push({
        id: `blocked-${src.id}-${block.start}-${block.end}`,
        srcId: src.id,
        title: `${src.name} (Blocked)`,
        start: block.start,
        end: block.end,
        color: src.blockedColor || globalSettings.blockedColor,
        textColor: '#ffffff',
        classNames: ['fc-event-blocked-block'],
        extendedProps: {
          isBlocked: true,
          srcId: src.id,
          srcName: src.name,
    ...
    console.log(`   🚫 ${src.name}: ${src.blockedDates.length} blocked date ranges`);
  });
...
function showDateEventCountTooltip(event, dateStr) {
  ...
  // Function to check if string looks like a URL or invalid
  const isUrlLike = (str) => {
    if (!str) return true;
  ...
  sources.filter(s => s.enabled).forEach(src => {
    src.bookings.forEach(booking => {
      ...
      if (checkDate >= start && checkDate < end) {
        ...
        const displayGuest = isInvalid ? 'Guest Name' : booking.guest;
...
function updateCalendar() {
  ...
  // Add blocked events as bars (NEW)
  const blockedEvents = generateBlockedEvents();
  allEvents = [...allEvents, ...blockedEvents];
...
function showTip(event, jsEvent) {
  ...
  if (b.isBlocked) {
    const startDisplay = fmtDate(b.blockStart);
    const endDisplay = b.blockEnd ? fmtDate(b.blockEnd) : '';
    tip.innerHTML = `
      <div class="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider mb-1">
      ...
      <div class="text-sm font-bold mb-1">🚫 ${b.srcName}</div>
      <div class="text-xs text-[#5b6380] dark:text-[#8b93b5] mb-1">${startDisplay} → ${endDisplay}</div>
      ${b.summary && b.summary !== 'Blocked' ? `<div class="text-xs text-[#5b6380] dark:text-[#8b93b5]">${b.summary}</div>` : ''}
    `;
    const x = jsEvent.clientX, y = jsEvent.clientY;
    const right = window.innerWidth - x > 260;
    tip.style.left = (right ? x + 16 : x - 246) + 'px';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (calEl) {
    calendar = new FullCalendar.Calendar(calEl, {
      ...
      eventContent: (arg) => {
        // Handle BLOCKED date blocks (NEW)
        if (arg.event.extendedProps && arg.event.extendedProps.isBlocked) {
          const name = arg.event.extendedProps.srcName || 'Blocked';
          ...
          switch (platform) {
            ...
            case 'booking':
              ...
              break;
            default:
              logoHtml = '<i data-lucide="x-circle" class="w-3 h-3" style="color:#fff;"></i>';
              break;
          }
      ...
      eventClick: ({ event }) => {
        const b = event.extendedProps;
        if (!b.isAvailable && !b.isBlocked) {
          openEditModal(b.id, b.srcId);
        }
        // Blocked and available events don't open edit modal (read-only)
      },
...
console.log('StaySync Loaded - Blocked dates now show as full calendar bars (red)');