...
class Room extends Model
{
    ...
    public function cloneForProperty(Property $property, int $sortOrder, bool $onlyDefaultTasks = true): self
    {
        ...
        foreach ($tasksToClone as $task) {
            ...
            // Clone media deeply to isolate properties
            foreach ($task->media as $m) {
                $rawPath = \App\Models\TaskMedia::normalizePath($m->getRawOriginal('url'));
                $newPath = $rawPath;
                ...
                if ($rawPath && \Illuminate\Support\Facades\Storage::disk('public')->exists($rawPath)) {
                    ...
                    $newPath = 'task-media/' . \Illuminate\Support\Str::random(40) . '.' . $ext;
                    \Illuminate\Support\Facades\Storage::disk('public')->copy($rawPath, $newPath);
                }
                
                $clonedTask->media()->create([
                    'type' => $m->type,
                    'url' => $newPath,
                    'thumbnail' => $m->type === 'image' ? $newPath : null,
                    'caption' => $m->caption,
