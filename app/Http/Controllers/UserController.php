...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $users = User::query()
                    ...
                    $q->where('users.id', $auth->id)
                        ...
                        ->orWhere(function ($qq) use ($auth) {
                            $qq->whereHas('roles', fn($r) => $r->where('name', 'housekeeper'))
                                ->where(function ($subQ) use ($auth) {
                                    $subQ->whereExists(function ($sub) use ($auth) {
                                        $sub->selectRaw(1)
                                            ->from('cleaning_sessions as cs')
                                            ->whereColumn('cs.housekeeper_id', 'users.id')
                                            ->where('cs.owner_id', $auth->id);
                                    })->orWhereExists(function ($sub) use ($auth) {
                                        $sub->selectRaw(1)
                                            ->from('housekeeper_owner as ho')
                                            ->whereColumn('ho.housekeeper_id', 'users.id')
                                            ->where('ho.owner_id', $auth->id);
                                    });
                                });
                        });
            ...
            ->when($status === 'active', fn($qry) => $qry->active())
            ->when($status === 'inactive', fn($qry) => $qry->inactive())
            ->orderBy('name')
            ->paginate(20)
            ->withQueryString();
    ...
    /**
     * Deactivate / terminate a cleaner account (Admin only).
     */
    public function deactivate(Request $request, User $user)
    {
        $authUser = $request->user();
        abort_unless($authUser && ($authUser->hasRole('admin') || $authUser->hasRole('owner')), 403, 'Only administrators and owners can deactivate user accounts.');
        abort_if($user->hasRole('admin'), 403, 'Administrator accounts cannot be deactivated.');
        abort_if($user->id === $authUser->id, 403, 'You cannot deactivate your own account.');
        abort_unless($user->is_active, 422, 'This account is already deactivated.');

        if ($authUser->hasRole('owner') && !$authUser->hasRole('admin')) {
            abort_if($user->hasAnyRole(['admin', 'owner', 'company']), 403, 'Owners can only modify housekeepers.');
            
            $isDirectlyOwned = $user->owner_id === $authUser->id;
            $isManaged = DB::table('housekeeper_owner')
                ->where('owner_id', $authUser->id)
                ->where('housekeeper_id', $user->id)
                ->exists();
            $hasSessions = DB::table('cleaning_sessions')
                ->where('owner_id', $authUser->id)
                ->where('housekeeper_id', $user->id)
                ->exists();

            abort_unless($isDirectlyOwned || $isManaged || $hasSessions, 403, 'This user is not assigned to you.');
        }

        $data = $request->validate([
            'termination_reason' => ['required', 'string', 'min:3', 'max:2000'],
        ], [
            'termination_reason.required' => 'Reason for termination is required.',
        ]);

        $user->deactivate($request->user(), $data['termination_reason']);

        if (function_exists('activity')) {
            activity()
                ->performedOn($user)
                ->causedBy($request->user())
                ->withProperties(['reason' => $data['termination_reason']])
                ->log('Cleaner account deactivated');
        }

        return back()->with('success', 'Cleaner account deactivated successfully.');
    }

    /**
     * Reactivate a cleaner account (Admin only).
     */
    public function reactivate(Request $request, User $user)
    {
        $authUser = $request->user();
        abort_unless($authUser && ($authUser->hasRole('admin') || $authUser->hasRole('owner')), 403, 'Only administrators and owners can reactivate cleaner accounts.');
        abort_unless(! $user->is_active, 422, 'This cleaner account is already active.');
        ...
        if (function_exists('activity')) {
            activity()
                ->performedOn($user)
                ->causedBy($request->user())
                ->log('Cleaner account reactivated');
        }

        return back()->with('success', 'Cleaner account reactivated successfully.');
    }
    ...
    public function create()
    {
        ...
        if ($user->hasRole('admin')) {
            ...
            $properties = Property::active()->where('owner_id', $user->id)->orderBy('name')->get();
        }
    ...
    public function edit(User $user)
    {
        ...
        if ($user->hasRole('housekeeper')) {
            $familiarities = \App\Models\CleanerInstructionFamiliarity::with('task')
        ...
        if ($authUser->id === $user->id) {
            $properties = Property::active()->where('owner_id', $authUser->id)->orderBy('name')->get();
            return view('users.edit', compact('user', 'properties', 'familiarities'));
        }
        ...
        if ($authUser->hasRole('admin')) {
            ...
            $properties = Property::active()->orderBy('name')->get();
            return view('users.edit', compact('user', 'owners', 'properties', 'familiarities'));
        }
    ...
    public function update(UserUpdateRequest $request, User $user)
    {
        return DB::transaction(function () use ($request, $user) {
            ...
            // Enforce owner_id based on role and provided input
            if (array_key_exists('owner_id', $data)) {
                $val = $data['owner_id'];
                if ($val === '' || $val == 0 || $val === null) {
                    if (!$authUser->hasRole('admin') && ($authUser->hasRole('company') || $authUser->hasRole('owner'))) {
                        $data['owner_id'] = $authUser->id; // "Direct"
                    } else {
                        $data['owner_id'] = null;
                    }
                } else {
                    if ($authUser->hasRole('owner') && !$authUser->hasRole('admin') && !$authUser->hasRole('company')) {
                        $data['owner_id'] = $authUser->id; // Enforce for standard owners
                    }
                }
            }

            // Log if required_instruction_views changed
            if (isset($data['preferences']['required_instruction_views'])) {
                $oldViews = $user->preferences['required_instruction_views'] ?? 3;
                $newViews = (int)$data['preferences']['required_instruction_views'];
                if ($oldViews !== $newViews) {
                    activity('users')
                        ...
                        ->log('Required Instruction Views Changed');
                }
            }
