...
export default function checklist(config = {}) {
    return {
        ...
        async handleToggle(event, button, extraData = {}) {
            ...
            // Defense-in-depth: Check Alpine component data for instruction requirement
            // This prevents the optimistic update flicker if the Alpine gate was somehow bypassed
            if (newChecked) {
                const taskEl = button.closest('[data-task-item]');
                if (taskEl && taskEl._x_dataStack) {
                    const alpineData = taskEl._x_dataStack[0];
                    if (alpineData && alpineData.requiresViewing && !alpineData.isFamiliar && !alpineData.instructionViewed) {
                        this.showError('You must view the instructions before completing this task.');
                        return;
                    }
                }
            }
            ...
            try {
                ...
                if (error.response?.status === 403 && error.response?.data?.code === 'TRAINING_REQUIRED') {
                    // It's a task-level training requirement failure. Just show the error.
                    this.showError(error.response.data.message || 'You must view the instructions before completing this task.');
                } else {
        ...
        updateLocalAndRerender(button, checked, data = null) {
            ...
            if (roomMatch) {
                ...
                // Search through ALL phases in property_tasks to find the task
                // This prevents state desync if user clicks a task in a non-active stage
                let updatedPhase = null;
                if (sd.property_tasks) {
                    for (const [phaseKey, tasks] of Object.entries(sd.property_tasks)) {
                        for (const task of tasks) {
                            if (task.id === taskId) {
                                ...
                                updatedPhase = phaseKey;
                                break;
                            }
                        }
                        if (updatedPhase) break;
                    }
                }

                // Recalculate counts for the updated phase
                if (updatedPhase && sd.counts?.[updatedPhase] && sd.property_tasks?.[updatedPhase]) {
                    const tasks = sd.property_tasks[updatedPhase];
                    sd.counts[updatedPhase].checked = tasks.filter(t => t.checklist_item?.checked).length;
                }
            }
        ...
        async saveProgress() {
            ...
            // BYPASS: If Admin is in Edit Report mode, save progress just means finishing the edit session
            const urlParams = new URLSearchParams(window.location.search);
            if (urlParams.get('edit_report') === '1' && renderer.sessionData.is_admin) {
            ...
            const sd = renderer.sessionData;
            const currentStage = renderer.viewOverride || sd.stage;
            ...
            if (currentStage === 'pre_cleaning' || currentStage === 'during_cleaning' || currentStage === 'post_cleaning') {
                ...
                const mandatory = tasks.filter(t => t.type !== 'instructions' && (t.type === 'verify' || t.type === 'inventory') && !t.checklist_item?.checked);
                const unviewedInstructions = tasks.filter(t => t.type === 'instructions' && !t.checklist_item?.instruction_viewed && !t.is_familiar);
                incompleteMandatory.push(...mandatory, ...unviewedInstructions);
            } else if (currentStage === 'rooms' || currentStage === 'rooms_first_half' || currentStage === 'rooms_second_half') {
                const rooms = sd.rooms || [];
                const skippedRoomIds = sd.session?.skipped_rooms || [];
                rooms.forEach(room => {
                    // Skip rooms that are in skipped_rooms
                    if (skippedRoomIds.includes(room.id)) return;
                    const tasksArr = Array.isArray(room.tasks) ? room.tasks : Object.values(room.tasks || {});
                    const mandatory = tasksArr.filter(t => t.type !== 'instructions' && (t.type === 'verify' || t.type === 'inventory') && !t.checklist_item?.checked);
                    const unviewedInstructions = tasksArr.filter(t => t.type === 'instructions' && !t.checklist_item?.instruction_viewed && !t.is_familiar);
                    incompleteMandatory.push(...mandatory, ...unviewedInstructions);
                });
            }

            if (incompleteMandatory.length > 0) {
                this.showError('Please complete all mandatory tasks and view all required instructions before advancing.');
                // Optionally scroll to the first incomplete task, or just scroll to bottom to see error
                window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
                return;
            }

            if (currentStage === 'photos') {
                let pendingPhotos = [];
                let rooms = sd.rooms || [];
                if (!Array.isArray(rooms)) rooms = Object.values(rooms);
                const photoCounts = sd.photo_counts || {};
                const skippedRoomIds = sd.session?.skipped_rooms || [];
                rooms.forEach(r => {
                    // Skip rooms that are in skipped_rooms
                    if (skippedRoomIds.includes(r.id)) return;
                    const count = photoCounts[r.id] || 0;
                    const min = renderer.getRoomMinPhotos ? renderer.getRoomMinPhotos(r) : 
                                ((Array.isArray(r.tasks) ? r.tasks : Object.values(r.tasks || {})).some(t => t.type !== 'instructions') ? Number(r.min_photos ?? 2) : 0);

                    
                    if (count < min) {
                        pendingPhotos.push(r.name);
                    }
                });
                
                if (pendingPhotos.length > 0) {
                    this.showError(`Please upload required photos for: ${pendingPhotos.join(', ')}`);
            ...
            try {
                // If the user is viewing a previous stage, "Next Stage" should just navigate forward locally
                if (renderer.viewOverride && renderer.viewOverride !== renderer.sessionData.stage) {
                    const stages = ['pre_cleaning', 'rooms', 'during_cleaning', 'photos', 'post_cleaning', 'summary'];
                    const viewIndex = stages.indexOf(renderer.viewOverride);
                    if (viewIndex >= 0 && viewIndex < stages.length - 1) {
                        const nextStage = stages[viewIndex + 1];
                        if (nextStage === renderer.sessionData.stage) {
                            renderer.viewOverride = null; // Reached current active stage
                        } else {
                            renderer.viewOverride = nextStage;
                        }
                ...
                // Forward edit_report param so backend allows stage advance on completed sessions
                const pageParams = new URLSearchParams(window.location.search);
                const isEditReport = pageParams.get('edit_report') === '1';
                const editSuffix = isEditReport ? '?edit_report=1' : '';
                const postBody = { current_stage: renderer.sessionData.stage };
                if (isEditReport) postBody.edit_report = '1';
                const response = await window.api.post(`/sessions/${sessionId}/advance-stage${editSuffix}`, postBody);
