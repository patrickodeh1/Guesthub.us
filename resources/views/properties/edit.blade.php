...
<x-app-layout>
    ...
    <x-card class="max-w-full bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-700/50">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 max-w-full">
                ...
                <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 max-w-full overflow-visible">
                    ...
                    <div class="md:col-span-2">
                        ...
                        @php
                            $currentTz = old('timezone', $property->timezone ?? config('app.timezone', 'America/New_York'));
                        @endphp
                        <x-form.select name="timezone" class="w-full" required x-ref="timezoneSelect">
                            ...
                            <option value="Pacific/Honolulu" @selected($currentTz === 'Pacific/Honolulu')>Hawaii (HST)</option>
                            <option value="America/Anchorage" @selected($currentTz === 'America/Anchorage')>Alaska (AKST/AKDT)</option>
                            <option value="America/Los_Angeles" @selected($currentTz === 'America/Los_Angeles')>Pacific Time (PST/PDT)</option>
                            <option value="America/Phoenix" @selected($currentTz === 'America/Phoenix')>Arizona (MST - No DST)</option>
                            <option value="America/Denver" @selected($currentTz === 'America/Denver')>Mountain Time (MST/MDT)</option>
                            <option value="America/Chicago" @selected($currentTz === 'America/Chicago')>Central Time (CST/CDT)</option>
                            <option value="America/New_York" @selected($currentTz === 'America/New_York')>Eastern Time (EST/EDT)</option>
                            <option value="America/Halifax" @selected($currentTz === 'America/Halifax')>Atlantic Time (AST/ADT)</option>
                            <option value="America/St_Johns" @selected($currentTz === 'America/St_Johns')>Newfoundland (NST/NDT)</option>
                            <option value="America/Sao_Paulo" @selected($currentTz === 'America/Sao_Paulo')>Brazil (BRT)</option>
                            <option value="Europe/London" @selected($currentTz === 'Europe/London')>London (GMT/BST)</option>
                            <option value="Europe/Paris" @selected($currentTz === 'Europe/Paris')>Paris, Berlin (CET/CEST)</option>
                            <option value="Europe/Helsinki" @selected($currentTz === 'Europe/Helsinki')>Helsinki, Kyiv (EET/EEST)</option>
                            <option value="Europe/Moscow" @selected($currentTz === 'Europe/Moscow')>Moscow (MSK)</option>
                            <option value="Asia/Dubai" @selected($currentTz === 'Asia/Dubai')>Dubai (GST)</option>
                            <option value="Asia/Karachi" @selected($currentTz === 'Asia/Karachi')>Karachi (PKT)</option>
                            <option value="Asia/Kolkata" @selected($currentTz === 'Asia/Kolkata')>India (IST)</option>
                            <option value="Asia/Bangkok" @selected($currentTz === 'Asia/Bangkok')>Bangkok (ICT)</option>
                            <option value="Asia/Singapore" @selected($currentTz === 'Asia/Singapore')>Singapore (SGT)</option>
                            <option value="Asia/Tokyo" @selected($currentTz === 'Asia/Tokyo')>Tokyo (JST)</option>
                            <option value="Australia/Sydney" @selected($currentTz === 'Australia/Sydney')>Sydney (AEST/AEDT)</option>
                            <option value="Pacific/Auckland" @selected($currentTz === 'Pacific/Auckland')>Auckland (NZST/NZDT)</option>
                        </x-form.select>
    ...
    <div class="mt-6">
        <x-card class="max-w-full bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-700/50">
            <div x-data="notificationSettingsPanel()" x-init="init()">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            Configure automated SMS alerts for cleaning events at this property.
                        </p>
                    </div>
                </div>
                ...
                <template x-if="successMsg">
                    <div class="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg text-sm text-green-700" x-text="successMsg" x-init="setTimeout(() => successMsg = '', 3000)"></div>
                </template>
                <template x-if="errorMsg">
                    <div class="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700" x-text="errorMsg"></div>
                </template>

                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {{-- Notification Toggles --}}
                    <div class="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-gray-50/40 dark:bg-gray-800/40">
                        <h4 class="font-semibold text-gray-800 dark:text-gray-200 mb-4">Notification Types</h4>
                        <div class="space-y-3">
                            <template x-for="setting in settingsList" :key="setting.key">
                                <label class="flex items-center justify-between gap-3 p-3 rounded-lg bg-white dark:bg-gray-900/60 border border-gray-100 dark:border-gray-700 cursor-pointer">
                                    <div>
                                        <p class="text-sm font-medium text-gray-800 dark:text-gray-100" x-text="setting.label"></p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400" x-text="setting.description"></p>
                                    </div>
                                    <div class="relative inline-flex items-center">
                                        <input type="checkbox" class="sr-only peer" :checked="settings[setting.key]" @change="toggleSetting(setting.key)">
                    ...
                    <div class="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-gray-50/40 dark:bg-gray-800/40">
                        <div class="flex items-center justify-between gap-3 mb-4">
                            <h4 class="font-semibold text-gray-800 dark:text-gray-200">Recipients</h4>
                            <span class="text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 dark:bg-indigo-500/10 dark:text-indigo-300 dark:border-indigo-500/20" x-text="recipients.length"></span>
                        </div>

                        {{-- Add Recipient Form --}}
                        <div class="mb-4 p-3 bg-white dark:bg-gray-900/60 border border-gray-100 dark:border-gray-700 rounded-lg">
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                <input type="text" x-model="newRecipientName" placeholder="Name" class="text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500">
                                <input type="text" x-model="newRecipientPhone" placeholder="Phone number" class="text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500">
                                <button type="button" @click="addRecipient()" :disabled="!newRecipientPhone || savingRecipient" class="px-3 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed">
                                    <span x-show="!savingRecipient">Add</span>
                                    <span x-show="savingRecipient">Adding...</span>
                                </button>
                            </div>
                        </div>
                            ...
                            <template x-for="recipient in recipients" :key="recipient.id">
                                <li class="flex items-center justify-between gap-3 p-2 rounded-lg bg-white dark:bg-gray-900/60 border border-gray-100 dark:border-gray-700">
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-medium text-gray-800 dark:text-gray-100 truncate" x-text="recipient.recipient_name || 'Unnamed'"></p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400" x-text="recipient.phone_number"></p>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <button type="button" @click="toggleRecipientActive(recipient)" class="text-xs px-2 py-1 rounded-full border" :class="recipient.active ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-50 text-gray-500 border-gray-200'" x-text="recipient.active ? 'Active' : 'Inactive'"></button>
                                        <button type="button" @click="removeRecipient(recipient)" class="text-xs text-red-600 hover:text-red-800">
                ...
                <div class="mt-6 border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-gray-50/40 dark:bg-gray-800/40">
                    <div class="flex items-center justify-between gap-3 mb-4">
                        <h4 class="font-semibold text-gray-800 dark:text-gray-200">Notification History</h4>
                        <button type="button" @click="loadHistory()" class="text-xs text-indigo-600 hover:text-indigo-800">
                            <span x-show="!loadingHistory">Refresh</span>
                            <span x-show="loadingHistory">Loading...</span>
                        </button>
                    </div>

                    <template x-if="!historyLoaded">
                        <button type="button" @click="loadHistory()" class="text-sm text-indigo-600 hover:text-indigo-800 underline">Load notification history</button>
                    </template>

                    <template x-if="historyLoaded && historyLogs.length === 0">
                        <p class="text-sm text-gray-500 dark:text-gray-400">No notifications sent yet.</p>
                    </template>
                                        ...
                                        <tr class="border-b border-gray-100 dark:border-gray-700/50">
                                            <td class="py-2 pr-3">
                                                <span class="px-2 py-0.5 rounded-full text-xs font-medium"
                                                    :class="{
                                                        'bg-blue-50 text-blue-700': log.notification_type === 'started',
                                                        'bg-green-50 text-green-700': log.notification_type === 'finished',
                                                        'bg-purple-50 text-purple-700': log.notification_type === 'photo',
                                                        'bg-amber-50 text-amber-700': log.notification_type === 'note',
                                                    }"
                                                    x-text="log.notification_type"></span>
                                            </td>
                                            <td class="py-2 pr-3 text-gray-700 dark:text-gray-300" x-text="log.recipient_phone"></td>
                                            <td class="py-2 pr-3">
                                                <span class="px-2 py-0.5 rounded-full text-xs font-medium"
                                                    :class="log.delivery_status === 'sent' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'"
                                                    x-text="log.delivery_status"></span>
                                            </td>
                                            <td class="py-2 pr-3 text-gray-500" x-text="log.sent_at ? new Date(log.sent_at).toLocaleString() : '—'"></td>
                                            <td class="py-2">
                                                <template x-if="log.delivery_status === 'failed'">
                                                    <button type="button" @click="resendNotification(log)" class="text-indigo-600 hover:text-indigo-800 underline">Resend</button>
                                                </template>
                                            </td>
                                        </tr>
                                    </template>
    ...
    <script>
        function notificationSettingsPanel() {
            return {
                settings: {
                    notify_cleaning_started: {{ $property->notify_cleaning_started ? 'true' : 'false' }},
                    notify_cleaning_finished: {{ $property->notify_cleaning_finished ? 'true' : 'false' }},
                    notify_photo_started: {{ $property->notify_photo_started ? 'true' : 'false' }},
                    notify_task_notes: {{ $property->notify_task_notes ? 'true' : 'false' }},
                },
                settingsList: [
                    { key: 'notify_cleaning_started', label: 'Cleaning Started', description: 'Notify when a cleaner starts a job' },
                    { key: 'notify_cleaning_finished', label: 'Cleaning Finished', description: 'Notify when a cleaner finishes a job' },
                    { key: 'notify_photo_started', label: 'Photo Session Started', description: 'Notify when the first photo is taken' },
                    { key: 'notify_task_notes', label: 'Task Notes', description: 'Notify when a cleaner adds a note to a task' },
                ],
                recipients: [],
                newRecipientName: '',
                newRecipientPhone: '',
                ...
                settingsUrl: @js(route('properties.notifications.update-settings', $property)),
                getSettingsUrl: @js(route('properties.notifications.settings', $property)),
                addRecipientUrl: @js(route('properties.notifications.add-recipient', $property)),
                historyUrl: @js(route('properties.notifications.history', $property)),
                ...
                async loadRecipients() {
                    try {
                        const res = await fetch(this.getSettingsUrl, { headers: { 'Accept': 'application/json' } });
                ...
                async toggleSetting(key) {
                    ...
                    try {
                        ...
                        this.settings[key] = !this.settings[key]; // revert
                    }
                },

                async addRecipient() {
                    if (!this.newRecipientPhone) return;
                    this.savingRecipient = true;
                    try {
                        const res = await fetch(this.addRecipientUrl, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                            ...
                            body: JSON.stringify({
                                phone_number: this.newRecipientPhone,
                                recipient_name: this.newRecipientName || null,
                            }),
                        });
                        const data = await res.json();
                        if (data.success) {
                            ...
                            this.successMsg = 'Recipient added.';
                        }
                    } catch (e) {
                        this.errorMsg = 'Failed to add recipient.';
                    }
                    this.savingRecipient = false;
                },

                async toggleRecipientActive(recipient) {
                    const url = @js(route('properties.notifications.update-recipient', [$property, ':id'])).replace(':id', recipient.id);
                    try {
                        ...
                        const data = await res.json();
                        if (data.success) {
                            recipient.active = data.recipient.active;
                        }
                    } catch (e) {
                        this.errorMsg = 'Failed to update recipient.';
                    }
                },

                async removeRecipient(recipient) {
                    if (!confirm('Remove this recipient?')) return;
                    const url = @js(route('properties.notifications.delete-recipient', [$property, ':id'])).replace(':id', recipient.id);
                    try {
                        const res = await fetch(url, {
                            method: 'DELETE',
                            headers: {
                        ...
                        if (data.success) {
                            ...
                            this.successMsg = 'Recipient removed.';
                        }
                    } catch (e) {
                        this.errorMsg = 'Failed to remove recipient.';
                    }
                },
                ...
                async loadHistory() {
                    ...
                    try {
                        ...
                        const data = await res.json();
                        if (data.success) {
                            this.historyLogs = data.logs.data || [];
                ...
                async resendNotification(log) {
                    const url = @js(route('properties.notifications.resend', [$property, ':id'])).replace(':id', log.id);
                    try {
                        const res = await fetch(url, {
                            method: 'POST',
                            headers: {
                        ...
                        const data = await res.json();
                        if (data.success) {
                            this.successMsg = 'Notification resent.';
                            this.loadHistory();
                        }
                    } catch (e) {
                        this.errorMsg = 'Failed to resend notification.';
                    }
                },
            };
        }
        ...
        >
            <x-card class="max-w-full bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-700/50">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <h3 class="text-base font-bold text-yellow-800 dark:text-yellow-200 uppercase">READ IMPORTANT INSTRUCTIONS</h3>
