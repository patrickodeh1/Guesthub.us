    ...
    <div class="space-y-4 w-full max-w-full">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 px-1 sm:px-0">
            <form method="get" class="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-2 flex-1 min-w-0 w-full">
                ...
                @if(auth()->user()->can('viewInactive', \App\Models\Property::class))
                    <div class="flex items-center ml-0 sm:ml-2 mt-2 sm:mt-0">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="show_inactive" value="true"
                                {{ request('show_inactive') ? 'checked' : '' }}
                                @change="this.closest('form').submit()"
        ...
        <div class="lg:hidden space-y-3">
            @forelse($properties as $property)
                ...
                @endphp
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 @if(!$property->is_active) opacity-60 @endif">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="font-semibold text-gray-900 dark:text-gray-100 truncate flex items-center gap-2">
                                ...
                                @if(!$property->is_active)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800 dark:bg-red-400/20 dark:text-red-300">
                                        Inactive
                                    </span>
                                @endif
                            </h3>
        ...
        <x-card class="!px-0 overflow-x-auto w-full hidden lg:block">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y dark:divide-gray-700">
                    @forelse($properties as $property)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 @if(!$property->is_active) opacity-60 @endif">
                            <td class="px-4 py-3">
                                @php
                                    $photoUrl = method_exists($property, 'getPhotoUrlAttribute')
                                        ? $property->photo_url
                            ...
                            <td class="px-4 py-3 font-medium flex items-center gap-2 h-full min-h-[4rem]">
                                {{ $property->name }}
                                @if(!$property->is_active)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800 dark:bg-red-400/20 dark:text-red-300">
                                        Inactive
