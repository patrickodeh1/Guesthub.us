            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2" x-data="taskAutocomplete({ suggestUrl: @js($suggestUrl) })"
                     @task-chosen="
                        ...
                        if (sel && $event.detail.task.type) sel.value = $event.detail.task.type;
                        const inst = document.getElementById('instructions');
                        if (inst && typeof $event.detail.task.instructions !== 'undefined' && $event.detail.task.instructions) inst.value = $event.detail.task.instructions;
                        const sporadic = document.querySelector('input[name=is_sporadic]');
                        if (sporadic && typeof $event.detail.task.is_sporadic !== 'undefined') sporadic.checked = $event.detail.task.is_sporadic;
                        const tId = document.getElementById('template_task_id');
                        if (tId && $event.detail.task.id) tId.value = $event.detail.task.id;
                        if ($event.detail.task.media && Array.isArray($event.detail.task.media)) {
                    ...
                    <input type="hidden" name="template_task_id" id="template_task_id" value="" />
                </div>

                {{-- Uploader (optional) --}}
                <div x-data="{ files: [], previews: [], existingMedia: [] }"
                     @template-media-loaded.window="existingMedia = $event.detail.map(m => ({...m, is_template: true}))"
                     x-init="
                    $watch('files', f => {
                    ...
                    <template x-if="existingMedia.length > 0">
                        <div class="mb-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
                            <template x-for="media in existingMedia" :key="media.id">
                                <div class="rounded border dark:border-gray-800 overflow-hidden relative group">
