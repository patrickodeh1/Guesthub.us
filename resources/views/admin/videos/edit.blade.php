<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    Edit Instructional Video: {{ $video->title }}
                </h2>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Update metadata, replace files, or change property assignments.
                </p>
            </div>
    ...
    <x-card>
        <form x-data="videoForm()" method="post" action="{{ route('admin.videos.update', $video) }}" enctype="multipart/form-data" @submit.prevent="submitForm($event)">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-1 space-y-6">
                    {{-- Video Uploader --}}
                    <div>
                        <x-form.label value="Video File" />
                        <div class="mt-1 border-2 border-dashed rounded-2xl p-5 flex flex-col items-center justify-center text-center cursor-pointer transition-colors bg-gray-50/40 dark:bg-gray-800/40"
                            :class="videoDrag ? 'border-indigo-500 bg-indigo-50/40 dark:bg-indigo-900/20' : 'border-gray-300 dark:border-gray-600'"
                            ...
                            <div class="w-full text-left p-2" x-show="videoName">
                                <div class="flex items-center gap-3 bg-indigo-50 dark:bg-indigo-950/40 p-3 rounded-xl border border-indigo-100 dark:border-indigo-900/40">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs text-gray-500" x-text="videoSize || 'Existing Video'"></p>
                                    </div>
                                </div>
                                <p class="mt-2 text-center text-xs text-gray-400">Drag &amp; drop or click to replace file.</p>
                            </div>

                            <input type="file" name="video" x-ref="videoFile" class="hidden" @change="previewVideo($event)" accept="video/mp4,video/quicktime,video/x-msvideo,video/webm,video/x-matroska" />
                        </div>
                    ...
                    {{-- Thumbnail Uploader --}}
                    <div>
                        <x-form.label value="Thumbnail Image" />
                        <div class="mt-1 border-2 border-dashed rounded-2xl p-5 flex flex-col items-center justify-center text-center cursor-pointer transition-colors bg-gray-50/40 dark:bg-gray-800/40"
                            :class="thumbDrag ? 'border-indigo-500 bg-indigo-50/40 dark:bg-indigo-900/20' : 'border-gray-300 dark:border-gray-600'"
                ...
                <div class="lg:col-span-2 space-y-4">
                    ...
                    <div>
                        ...
                        <x-form.input name="title" class="w-full" required :value="old('title', $video->title)"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                            ...
                            placeholder="Provide a description of the procedures in the video... (Markdown supported)">{{ old('description', $video->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <x-form.select name="category" class="w-full" required>
                                ...
                                <option value="general" @selected(old('category', $video->category) === 'general')>General Guidelines</option>
                                <option value="room-specific" @selected(old('category', $video->category) === 'room-specific')>Room Specific</option>
                                <option value="equipment" @selected(old('category', $video->category) === 'equipment')>Equipment</option>
                                <option value="safety" @selected(old('category', $video->category) === 'safety')>Safety</option>
                                <option value="process" @selected(old('category', $video->category) === 'process')>Process</option>
                            </x-form.select>
                        ...
                        <div>
                            ...
                            <x-form.input name="duration_seconds" type="number" min="0" class="w-full" :value="old('duration_seconds', $video->duration_seconds)"
                    ...
                    <div class="pt-2">
                        <label class="flex items-center gap-3 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                            <input type="checkbox" name="is_published" value="1"
                                ...
                                @checked(old('is_published', $video->is_published) == '1')>
                            <div>
                                <span class="font-semibold text-gray-900 dark:text-gray-100">Published</span>
                    ...
                    <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <div class="space-y-4">
                            ...
                            <div x-show="isPreArrival" x-cloak class="pl-8 space-y-4">
                                <div>
                                    <label class="flex items-center gap-3 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                                        <input type="checkbox" name="is_required_before_start" value="1"
                                            class="rounded border-gray-300 dark:border-gray-700 text-indigo-600 focus:ring-indigo-500 w-5 h-5"
                                            @checked(old('is_required_before_start', $video->is_required_before_start) == '1')>
                                        <div>
                                ...
                                <div class="space-y-4 mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                                    <label class="flex items-center gap-3 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                                        <input type="checkbox" name="is_required_during_task" value="1"
                                            class="rounded border-gray-300 dark:border-gray-700 text-indigo-600 focus:ring-indigo-500 w-5 h-5"
                                            @checked(old('is_required_during_task', $video->is_required_during_task) == '1')>
                                        <div>
                            ...
                            <div x-show="isPreArrival || formData?.is_required_during_task" class="pl-8 pt-2">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-form.input type="number" name="required_views" class="w-full" min="1" value="{{ old('required_views', $video->required_views ?: 1) }}" />
                                    ...
                                    <div>
                                        ...
                                        <x-form.input type="number" name="completion_threshold_percent" class="w-full" min="1" max="100" value="{{ old('completion_threshold_percent', $video->completion_threshold_percent ?: 90) }}" />
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-form.select name="training_frequency" class="w-full">
                                            <option value="once_ever" @selected(old('training_frequency', $video->training_frequency) === 'once_ever')>Once Ever</option>
                                            <option value="once_per_property" @selected(old('training_frequency', $video->training_frequency) === 'once_per_property')>Once Per Property</option>
                                            <option value="once_per_assignment" @selected(old('training_frequency', $video->training_frequency) === 'once_per_assignment')>Once Per Assignment</option>
                                            <option value="every_assignment" @selected(old('training_frequency', $video->training_frequency) === 'every_assignment')>Every Assignment</option>
                                        </x-form.select>
                                    </div>
                                    <div>
                                        <!-- Empty column to maintain grid layout, or we can just remove the div -->
                                    </div>
                                </div>
                                ...
                                <div class="pt-2 border-t border-gray-100 dark:border-gray-700 mt-4">
                                    <label class="flex items-center gap-3 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                                        <input type="checkbox" name="bump_version" value="1"
                                            class="rounded border-gray-300 dark:border-gray-700 text-orange-500 focus:ring-orange-500 w-5 h-5">
                                        <div>
                                            <span class="font-semibold text-gray-900 dark:text-gray-100">Bump Version (Require Retraining)</span>
                                            <p class="text-xs text-gray-500 dark:text-gray-400">Check this if the content has changed significantly and all cleaners must watch the new version, regardless of previous completion.</p>
                                        </div>
                                    </label>
                                </div>
                            </div>
                                ...
                                @foreach ($properties as $property)
                                    <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer hover:text-indigo-600 transition-colors p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50">
                                        <input type="checkbox" name="properties[]" value="{{ $property->id }}"
                                            ...
                                            @checked(in_array($property->id, old('properties', $attachedPropertyIds)))>
    ...
    <script>
        function videoForm() {
            return {
                isPreArrival: {{ old('is_pre_arrival', $video->is_pre_arrival) ? 'true' : 'false' }},
                videoDrag: false,
                videoName: @js($video->video_file_path ? basename($video->video_file_path) : null),
                videoSize: null,
                videoDuration: null,
                uploading: false,
                uploadProgress: 0,
                ...
                thumbDrag: false,
                thumbUrl: @js($video->thumbnail_path ? $video->thumbnail_url : null),
                ...
                async submitForm(event) {
                    ...
                    this.uploadProgress = 0;
                    this.uploadError = null;
                    this.uploadSpeed = 'Preparing to update...';
                    this.uploadEta = null;
                    
                    let fileToUpload = this.$refs.videoFile.files?.[0];
                    
                    // Process video with FFmpeg.wasm only if a new video is uploaded
                    if (fileToUpload) {
                        try {
